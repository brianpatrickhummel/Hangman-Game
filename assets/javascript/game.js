...
window.onkeyup = function (event) {
	if ((event.keyCode == (32))) {
		...
		document.onkeypress = function (event) {	   //capture keypress						 		
			var x = event.which;
			var userGuess = "";
			...
			if (((lettersGuessed.indexOf(userGuess)) !== -1) || ((answerArray.indexOf(userGuess)) !== -1)) {
				...
				else {													//       ALTERNATE METHOD, ACCOUNTING FOR MULTIPLE LETTERS AT DIFF INDEXES																	//     var blanks = computerWord.split("").map(function (){return "_"})
					for (var k = 0; k < computerWord.length; k++) {    	//	   var index = computerWord.indexOf(userGuess)
						if (userGuess === computerWord[k]) {			//	   while (index > -1){
